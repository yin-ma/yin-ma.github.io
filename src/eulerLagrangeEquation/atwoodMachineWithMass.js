export class AtwoodMachineWithMass {
  constructor(p5) {
    this.p5 = p5;

    p5.describe(
      `atwood machine with mass
      $$L = T - V = \\frac{1}{2}m_{1}\\dot{x}^{2} + \\frac{1}{2}m_{2}\\dot{x}^{2} + \\frac{1}{4}M\\dot{x}^{2} + m_{1}gx+m_{2}gl-m_{2}gx$$ &nbsp;
    ...
    this.timer = 0;
    this.pulleyX = -this.hangingRadius + 5;
    this.pulleyY = this.l;
    this.pulleyTheta = 0;
    this.pulleyAngularVelocity = 0;
    this.pulleyAngularAcceleration = 0;
  }
  ...
  update() {
    ...
    this.bob1.acceleration = (this.bob1.mass - this.bob2.mass) / (this.bob1.mass + this.bob2.mass + 0.5 * this.M) * this.gravity;
    ...
    this.pulleyAngularAcceleration = this.bob1.acceleration / this.hangingRadius;
    this.pulleyAngularVelocity += this.pulleyAngularAcceleration;
    this.pulleyTheta += this.pulleyAngularVelocity;

    this.pulleyY -= this.l;
    this.pulleyX = (this.hangingRadius - 10) * Math.sin(this.pulleyTheta);
    this.pulleyY = (this.hangingRadius - 10) * Math.cos(this.pulleyTheta);
    this.pulleyY += this.l;
